...
const FeatureCard: React.FC<{ icon: React.ReactElement; title: string; description: string }> = ({ icon, title, description }) => (
    <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700 rounded-2xl p-6 transition-all duration-300 hover:border-slate-600 hover:-translate-y-1 h-full">
        <div className="bg-indigo-600/10 text-indigo-400 rounded-lg w-12 h-12 flex items-center justify-center mb-4">
...
const HowItWorksStep: React.FC<{ step: number; title: string; description: string }> = ({ step, title, description }) => (
    <div className="flex flex-col items-center text-center md:flex-row md:text-left md:items-start space-y-4 md:space-y-0 md:space-x-4">
        <div className="flex-shrink-0 w-12 h-12 flex items-center justify-center bg-slate-800/50 border border-slate-700 rounded-full font-bold text-lg text-indigo-400">
            {step}
        </div>
        <div>
...
const TestimonialCard: React.FC<{ quote: string; name: string; role: string; avatar: string }> = ({ quote, name, role, avatar }) => (
    <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700 rounded-2xl p-8 h-full flex flex-col justify-between">
...
const SocialProofLogos: React.FC = () => {
    ...
    return (
        <div className="mt-12">
            <p className="text-center text-sm text-slate-500 font-medium mb-6">USED BY PROFESSIONALS AT:</p>
            <div className="flex justify-center items-center space-x-6 sm:space-x-8 md:space-x-12 opacity-60 grayscale flex-wrap">
                {icons.map(({ Icon, name }) => (
                    <Icon key={name} className="h-6 md:h-7 text-slate-400" title={name}/>
                ))}
            </div>
        </div>
    );
            ...
            <div className="relative z-10">
                ...
                <main>
                    ...
                    <section className="min-h-screen flex items-center justify-center pt-24 pb-16 text-center">
                        <div className="container mx-auto px-4">
                            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white tracking-tighter leading-tight mb-6 animate-fade-in-down">
                                Spend your time <span className="bg-gradient-to-r from-indigo-400 to-cyan-400 bg-clip-text text-transparent">closing deals</span>, not chasing unqualified leads.
                                <br />
                                Let our AI do the <span className="bg-gradient-to-r from-indigo-400 to-cyan-400 bg-clip-text text-transparent">filtering for you</span>.
                            </h1>
                            <p className="text-base sm:text-lg md:text-xl text-slate-400 max-w-3xl mx-auto mb-10 animate-fade-in-up delay-100">
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4 animate-fade-in-up delay-200">
                                <Link to="/register" className="w-full sm:w-auto px-8 py-4 text-base font-bold text-white bg-indigo-600 rounded-lg hover:bg-indigo-500 transition-transform hover:scale-105 duration-300 shadow-lg shadow-indigo-600/40">
                                    Get Started for Free
                                </Link>
                                <button className="w-full sm:w-auto px-8 py-4 text-base font-bold text-white bg-transparent border-2 border-slate-700 rounded-lg hover:bg-slate-800 transition-colors duration-300">
                                    View Demo
                                </button>
                            </div>
                        ...
                        <div className="container mx-auto px-4 text-center">
                            ...
                            <div className="max-w-4xl mx-auto">
                                <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700 rounded-2xl p-8 text-center">
                                    ...
                                    <p className="text-slate-400 mb-6">From solo creators to large agencies, we have a plan that fits your needs.</p>
                                     <Link to="/pricing" className="font-bold text-white bg-transparent border-2 border-indigo-500 rounded-lg px-6 py-3 hover:bg-indigo-500 hover:text-white transition-colors duration-300">
                                        Compare Plans
                                    </Link>
                                </div>
                            </div>
                    ...
                    <section className="py-16 md:py-24">
                         <div className="container mx-auto px-4 text-center">
                             <div className="max-w-3xl mx-auto bg-gradient-to-r from-indigo-500/20 to-sky-500/20 rounded-3xl p-6 sm:p-10 border border-slate-700">
                                ...
                                <p className="text-slate-300 mb-8">
                                 ...
                                 <Link to="/register" className="px-8 py-4 text-base font-bold text-white bg-indigo-600 rounded-lg hover:bg-indigo-500 transition-transform hover:scale-105 duration-300 shadow-lg shadow-indigo-600/40">
                                    Sign Up Free
                                 </Link>
                             </div>
                         </div>
